...
local newItem = {
  class = "thebfc", -- the item class name needs to be the same as the one in the xml file
  percent = 12, -- the percentage will need to be substracted from an existing field in the category
}

local categoryToAdjust = FindInTable(ItemSpawnerManager.itemCategories, "category", "RandomDrinkVendingMachineContent")
local categoryItemToAdjust = FindInTable(categoryToAdjust.classes, "class", "EnergyDrinkCan") -- could also search for fitting "class" instead "category"
local categoryItemToAdjust1 = FindInTable(categoryToAdjust.classes, "class", "AmcoinLedger")
