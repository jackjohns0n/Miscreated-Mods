...
local newItem = {
  class = "LeatherJacketNoSmoke", -- the item class name needs to be the same as the one in the xml file
  percent = 20, -- the percentage will need to be substracted from an existing field in the category
}

local categoryToAdjust = FindInTable(ItemSpawnerManager.itemCategories, "category", "RandomLeatherJacket")
local categoryItemToAdjust = FindInTable(categoryToAdjust.classes, "class", "LeatherJacketYellow") -- could also search for fitting "class" instead "category"
