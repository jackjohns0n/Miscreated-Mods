...
local newItem = {
category = "RandomPistol", -- the item class name needs to be the same as the one in the xml file
percent = 40, -- the percentage will need to be substracted from an existing field in the category
}

local categoryToAdjust = FindInTable(ItemSpawnerManager.itemCategories, "category", "RandomBruteLoot")
local categoryItemToAdjust = FindInTable(categoryToAdjust.group, "class", "AmcoinLedger") -- could also search for fitting "class" instead "category"
local categoryItemToAdjust2 = FindInTable(categoryToAdjust.group, "category", "RandomCraftingGuide")

-- Add new item at the end of list
table.insert(categoryToAdjust.group, newItem)
