...
local categoryToAdjust = FindInTable(ItemSpawnerManager.itemCategories, "category", "RandomHospitalContentBig")
local categoryItemToAdjust = FindInTable(categoryToAdjust.classes, "class", "WorkLight") -- could also search for fitting "class" instead "category"
local categoryItemToAdjust2 = FindInTable(categoryToAdjust.classes, "class", "small_generator")
local categoryItemToAdjust3 = FindInTable(categoryToAdjust.classes, "class", "powered_generator_small")
local categoryItemToAdjust4 = FindInTable(categoryToAdjust.classes, "category", "RandomHospitalClothes")
