...
local newItem = {
  category = "RandomCraftedWeapons", -- the item class name needs to be the same as the one in the xml file
  percent = 35, -- the percentage will need to be substracted from an existing field in the category
}

local categoryToAdjust = FindInTable(ItemSpawnerManager.itemCategories, "category", "RandomToolboxContent")
local categoryItemToAdjust = FindInTable(categoryToAdjust.classes, "class", "Nails") -- could also search for fitting "class" instead "category"
