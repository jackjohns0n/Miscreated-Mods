...
local newItem = {
  category = "RandomCouchContent", -- the item class name needs to be the same as the one in the xml file
  percent = 90, -- the percentage will need to be substracted from an existing field in the category
}

local categoryToAdjust = FindInTable(ItemSpawnerManager.itemCategories, "category", "SmallHidingPlaceContent")
...
local categoryItemToAdjust2 = FindInTable(categoryToAdjust.classes, "category", "RandomTrashContent")
