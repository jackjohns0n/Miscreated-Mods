...
local newItem = {
  ...
  percent = 100, -- the percentage will need to be substracted from an existing field in the category
}

local categoryToAdjust = FindInTable(ItemSpawnerManager.itemCategories, "category", "RandomBPart")
local categoryItemToAdjust = FindInTable(categoryToAdjust.classes, "category", "RandomCampingBPart") -- could also search for fitting "class" instead "category"
local categoryItemToAdjust2 = FindInTable(categoryToAdjust.classes, "category", "RandomGnome") 
